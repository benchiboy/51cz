...
func New(db *sql.DB, level int) *UsersList {
	if db==nil{
		log.Println(SQL_SELECT,"Database is nil")
		return nil
	}
	return &UsersList{DB: db, Total: 0, Userss: make([]Users, 0), Level: level}
}
...
func NewUrl(url string, level int) *UsersList {
	var err error
...
func (r *UsersList) GetTotal(s Search) (int, error) {
	var where string
	l := time.Now()
...
func (r UsersList) Get(s Search) (*Users, error) {
	var where string
	l := time.Now()
...
func (r *UsersList) GetList(s Search) ([]Users, error) {
	var where string
	l := time.Now()
...
func (r *UsersList) GetExt(s Search) (map[string]string, error) {
	var where string
	l := time.Now()
...
func (r UsersList) Insert(p Users) error {
	l := time.Now()
	exeSql := fmt.Sprintf("Insert into  users(user_id,user_name,phone,head_image,version)  values(?,?,?,?,?,?,?,?)")
	if r.Level == DEBUG {
		log.Println(SQL_INSERT, exeSql)
	}
	_, err := r.DB.Exec(exeSql, p.UserId,p.UserName,p.Phone,p.HeadImage,p.Version)
	if err != nil {
		log.Println(SQL_ERROR, err.Error())
		return err
	}
...
func (r UsersList) InsertEntity(p Users, tr *sql.Tx) error {
	l := time.Now()
...
func (r UsersList) InsertMap(m map[string]interface{},tr *sql.Tx) error {
	l := time.Now()
	var colNames, colTags string
	valSlice := make([]interface{}, 0)
	for k, v := range m {
...
func (r UsersList) UpdataEntity(keyNo string,p Users,tr *sql.Tx) error {
	l := time.Now()
	var colNames string
	valSlice := make([]interface{}, 0)
...
func (r UsersList) UpdateMap(keyNo string, m map[string]interface{},tr *sql.Tx) error {
	l := time.Now()
...
func (r UsersList) Delete(keyNo string,tr *sql.Tx) error {
	l := time.Now()
